...
import { resetSubsetSumTableState, updateSubsetSumCellVisual, flushSubsetSumStepsNow, waitForSubsetSumPlaybackDone } from '../../../../gameutils/utils/blockly/algorithms/subset_sum/subsetSumStateManager';
import { resetCoinChangeTableState, updateCoinChangeCellVisual, flushCoinChangeStepsNow, waitForCoinChangePlaybackDone } from '../../../../gameutils/utils/blockly/algorithms/coin_change/coinChangeStateManager';
import { resetAntDpTableState, updateAntDpCellVisual, flushAntDpStepsNow, waitForAntDpPlaybackDone, waitForAntDpVisualIdle } from '../../../../gameutils/utils/blockly/algorithms/ant_dp/antDpStateManager';
import {
...
`;
            ...
            try {
                ...
                if (isCoinChange) {
                    ...
                    // EXTRA ROBUSTNESS: Explicitly check for Graph Algorithm function calls (DFS, BFS, etc)
                    // to ensure we capture the variable name even if the generic pattern above missed something subtle.
    ...
    if (typeof result === 'undefined' || result === null) {
      ...
      `;
        ...
        `;
                ...
                const isRopePartitionCheck = currentLevel.gameType === 'rope_partition' || currentLevel.appliedData?.type === 'BACKTRACKING_ROPE_PARTITION';
                ...
                // Build fresh state object explicitly to avoid async state update race conditions
                ...
                console.log("🔍 State for victory check (Freshly Constructed):", freshState);
                console.log("🔍 testCaseResult in freshState:", freshState.testCaseResult);

                // Pass the fresh state explicitly to the victory checker
                ...
                if (levelCompleted) {
                    ...
                    // --- ROBUST ANIMATION CHECKER (Success Path) ---
                    ...
                    if (currentScene && currentScene.sys && currentScene.sys.isActive()) {
                        ...
                        // Fallback: Try global search or delay
                        let fallbackScene = null;
                        try {
                            if (window.game && window.game.scene && window.game.scene.scenes) {
                                fallbackScene = window.game.scene.scenes.find(s => s.sys && s.sys.isActive() && s.sys.settings.key === 'GameScene');
                            }
                        ...
                        if (fallbackScene) {
                            ...
                            if (isCinematicLevel) {
                                await new Promise((resolve) => {
                                    playCombatSequence(fallbackScene, true, () => { resolve(); });
                                });
                            } else {
                                await new Promise(r => setTimeout(r, 800));
                            }
                    ...
                    // --- CRITICAL SECTION: State Updates AFTER Animation ---
                    setIsCompleted(true);
                    setGameState("completed");

                    // Retrieve current state for scoring
                    const execFinalState = getCurrentGameState();

                    // Calculate Test Case Bonus
                    let testCaseBonus = 0;
                    // Use local testCaseResult if available, otherwise fallback to state
                    const resultToCheck = testCaseResult || execFinalState.testCaseResult;
                    ...
                    // ระบบคะแนน: ใช้ patternTypeId จาก execFinalState ถ้าไม่มีให้ fallback หาใน goodPatterns
                    let patternTypeId = execFinalState.patternTypeId;
                    ...
                    // Fix Big O Scoring: Extract targetBigO from hintData explicitly first (prioritize bestPatternBigO)
                ...
                if (levelCompleted) {
                    ...
                    // State updates moved to post-animation callback
                    // setIsCompleted(true);
                    // setGameState("completed");
                    ...
                    // setFinalScore(scoreData); // Moved to post-animation callback
                    ...
                    if (currentScene && currentScene.sys && currentScene.sys.isActive()) {
                        ...
                        // Fallback if no scene or scene inactive
                        let fallbackScene = null;
                        try {
                            if (window.game && window.game.scene && window.game.scene.scenes) {
                                fallbackScene = window.game.scene.scenes.find(s => s.sys && s.sys.isActive() && s.sys.settings.key === 'GameScene');
                        ...
                        if (fallbackScene) {
                            ...
                            if (isCinematicLevel) {
                                await new Promise((resolve) => {
                                    playCombatSequence(fallbackScene, true, () => {
                                        resolve();
                                    });
                                });
                            } else {
                            ...
                            const victoryType = currentLevel.goalType === "ช่วยคน" ? 'rescue' : 'normal';
                            showVictory(fallbackScene, victoryType);
                            if (!isCinematicLevel) {
                                await new Promise(r => setTimeout(r, 2000));
                            }
                        } else {
                    ...
                    // --- CRITICAL SECTION: ONLY REACHED AFTER ANIMATION/WAIT FINISHES ---
                    setIsCompleted(true);
                    setGameState("completed");
                    setFinalScore(scoreData);

                    if (isPreview) {
                        if (patternId && onUnlockPattern) await onUnlockPattern(patternId);
                        else if (onUnlockPattern) {
                            const matchedPattern = goodPatterns.find(p => p.pattern_id === patternId) || goodPatterns[0];
                            if (matchedPattern) await onUnlockPattern(matchedPattern.pattern_id);
                        }
                        if (onUnlockLevel && currentLevel) await onUnlockLevel(currentLevel.level_id);
                    } else {
                        setShowProgressModal(true);
                    }
