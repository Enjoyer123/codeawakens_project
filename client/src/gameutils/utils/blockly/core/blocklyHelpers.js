...
import { highlightNode, showCurrentPath, showMSTEdges, showMSTEdgesFromList as showMSTEdgesFromListVisual, highlightKruskalEdge, showKruskalRoot, clearKruskalVisuals } from '../graph/blocklyDfsVisual';
import { updateDijkstraVisited, updateDijkstraPQ } from '../graph/dijkstraStateManager';
import {
  getPlayerCoins,
  addCoinToPlayer,
  swapPlayerCoins,
...
import { antMaxWithVisual, showAntDpFinalPath } from '../algorithms/ant_dp/blocklyAntDpVisual';
import {
  addWarriorToSide1,
  addWarriorToSide2,
...
export async function moveAlongPath(path) {
  ...
  if (currentState.currentScene) {
    const { clearScanningHighlights } = await import('../graph/blocklyDfsVisual');
