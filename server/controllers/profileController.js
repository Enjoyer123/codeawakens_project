...
exports.updateUsername = async (req, res) => {
  ...
  try {
    ...
    res.status(error.status || 500).json({ message: error.message || "Failed to update username" });
  }
};
...
exports.uploadProfileImage = async (req, res) => {
  ...
  try {
    ...
    const { imageUrl, user } = await profileService.uploadProfileImage(clerkId, req.file);
    console.log(`[PROFILE] Success: Profile image uploaded for User ${clerkId}.`);
    res.json({ message: "Profile image uploaded successfully", profileImageUrl: imageUrl, user });
  } catch (error) {
...
exports.deleteProfileImage = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Profile image deleted successfully", profileImageUrl });
  } catch (error) {
    console.error(`[ERROR] Failed to delete profile image for User ${clerkId}:`, error.message);
    res.status(error.status || 500).json({ message: error.message || "Failed to delete profile image" });
  }
};
...
exports.getUserByClerkId = async (req, res) => {
  ...
  try {
    ...
    res.status(error.status || 500).json({ message: error.message || "Error fetching user details" });
  }
};
...
exports.saveUserProgress = async (req, res) => {
  ...
  try {
    ...
    res.status(error.status || 500).json({ message: error.message || "Error saving user progress" });
  }
};
...
exports.checkAndAwardRewards = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "Rewards checked and awarded successfully", ...result });
  } catch (error) {
    console.error(`[ERROR] Failed to check rewards for User ${clerkId}:`, error.message);
    res.status(error.status || 500).json({ message: error.message || "Error checking and awarding rewards" });
  }
};