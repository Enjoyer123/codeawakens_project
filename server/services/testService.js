...
export const getTestsByType = async (type, clerkUserId) => {
  ...
  if (type === "pre") testType = "PreTest";
    ...
    const err = new Error("Invalid test type");
    err.status = 400;
    throw err;
  }
    ...
    if (user) {
      ...
      if (requiredLevels.length > 0) {
        ...
        const missingLevels = requiredLevels.filter(
          (l) => !completedIds.includes(l.level_id),
        );
  ...
  if (testType === "PreTest") {
    ...
    for (const part of parts) {
      const partTests = await prisma.test.findMany({
        where: { test_type: "PreTest", part, is_active: true },
...
export const submitTest = async (type, answers, clerkId) => {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { user_id: true, pre_score: true, post_score: true },
  });
  if (!user) {
  ...
  if (testType === "PreTest" && user.pre_score !== null) {
    const err = new Error("You have already completed the Pre-Test.");
    err.status = 400;
    throw err;
  }
  if (testType === "PostTest" && user.post_score !== null) {
    const err = new Error("You have already completed the Post-Test.");
    err.status = 400;
    throw err;
  }
  if (!answers || !Array.isArray(answers)) {
    const err = new Error("Invalid answers format");
    err.status = 400;
    throw err;
  }
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    const dbTests = await tx.test.findMany({
      where: { test_type: testType, is_active: true },
      include: { choices: true },
    });
    ...
    for (const ans of answers) {
      ...
      if (!test) continue;
      const selectedChoice = test.choices.find(
        (c) => c.test_choice_id === ans.choice_id,
      );
      ...
      userTestCreates.push(
        tx.userTest.upsert({
          where: {
            user_id_test_id: { user_id: user.user_id, test_id: ans.test_id },
          },
          update: {
    ...
    if (testType === "PreTest") {
      ...
      console.log(
        `Scoring: ${score}/${totalQuestions} (${percentage.toFixed(1)}%) => Skill: ${skillLevel}`,
      );
    }
  ...
  return {
    ...
    details: { p1: result.scoreP1, p2: result.scoreP2, p3: result.scoreP3 },
